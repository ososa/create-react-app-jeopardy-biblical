...
type TermsOfUseScreenProps = {
    navigation: NativeStackNavigationProp<RootStackParamList, 'TermsOfUse' | 'Login' | 'Instructions'>;
};
...
export const TermsOfUseScreen: React.FC<TermsOfUseScreenProps> = ({ navigation }) => {
    ...
    return (
        <LinearGradient colors={['#001B3A', '#0D3B66', '#1A5276']} style={styles.container}>
            ...
            <Helmet>
                <title>{t('footer.terms')} - Tribiblia</title>
            </Helmet>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                <View style={[styles.contentContainer, { maxWidth: isLargeScreen ? 800 : '90%' }]}>
                    ...
                    <View style={styles.titleContainer}>
                        ...
                        <Text style={styles.sectionTitle}>{t('pages.terms.title')}</Text>
                        <View style={[styles.titleLine, { backgroundColor: 'rgb(16, 223, 224)' }]} />
                    </View>

                    <View style={styles.bodyContainer}>
                        <Text style={[styles.bodyText, { textAlign: isLargeScreen ? 'justify' : 'left' }]}>
                            {renderBoldText(t('pages.terms.body1'))}
                            {"\n\n"}
                            {renderBoldText(t('pages.terms.body2'))}
                            {"\n\n"}
                            {renderBoldText(t('pages.terms.body3'))}
                        </Text>
                    </View>
