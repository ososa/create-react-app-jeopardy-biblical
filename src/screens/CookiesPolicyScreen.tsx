...
type CookiesPolicyScreenProps = {
    navigation: NativeStackNavigationProp<RootStackParamList, 'CookiesPolicy' | 'Login' | 'Instructions'>;
};
...
const renderBoldText = (text: string) => {
    ...
    return parts.map((part, index) => {
        if (index % 2 === 1) {
            return <Text key={index} style={{ fontFamily: 'Mulish-Bold', fontWeight: 'bold' }}>{part}</Text>;
        }
        return <Text key={index}>{part}</Text>;
    });
};

export const CookiesPolicyScreen: React.FC<CookiesPolicyScreenProps> = ({ navigation }) => {
    ...
    return (
        <LinearGradient colors={['#001B3A', '#0D3B66', '#1A5276']} style={styles.container}>
            ...
            <Helmet>
                <title>{t('footer.cookies')} - Tribiblia</title>
            </Helmet>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                <View style={[styles.contentContainer, { maxWidth: isLargeScreen ? 800 : '90%' }]}>
                    ...
                    <View style={styles.titleContainer}>
                        ...
                        <Text style={styles.sectionTitle}>{t('pages.cookies.title')}</Text>
                        <View style={[styles.titleLine, { backgroundColor: 'rgb(16, 223, 224)' }]} />
                    </View>

                    <View style={styles.bodyContainer}>
                        <Text style={[styles.bodyText, { textAlign: isLargeScreen ? 'justify' : 'left' }]}>
                            {renderBoldText(t('pages.cookies.body1'))}
                            {"\n\n"}
                            {renderBoldText(t('pages.cookies.body2'))}
                            {"\n\n"}
                            {renderBoldText(t('pages.cookies.body3'))}
                        </Text>
                    </View>
